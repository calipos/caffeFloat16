...
__global__ void im2col_gpu_kernel_half(const int n, const __half* data_im,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    const int h_offset = h_col * stride_h - pad_h;
    const int w_offset = w_col * stride_w - pad_w;
    __half* data_col_ptr = data_col;
    data_col_ptr += (c_col * height_col + h_col) * width_col + w_col;
    const __half* data_im_ptr = data_im;
    data_im_ptr += (c_im * height + h_offset) * width + w_offset;
    for (int i = 0; i < kernel_h; ++i) {
      for (int j = 0; j < kernel_w; ++j) {
        int h_im = h_offset + i * dilation_h;
        ...
        *data_col_ptr =
            (h_im >= 0 && w_im >= 0 && h_im < height && w_im < width) ?
            data_im_ptr[i * dilation_h * width + j * dilation_w] : __float2half(0.0);
        data_col_ptr += height_col * width_col;
      }
    }
  }
...
void im2col_gpu_half(const __half* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w,
  ...
  im2col_gpu_kernel_half<<<CAFFE_GET_BLOCKS(num_kernels),
...
__global__ void col2im_gpu_kernel_half(const int n, const __half* data_col,
    const int height, const int width, const int channels,
    const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
...
void col2im_gpu_half(const __half* data_col, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
  ...
  // NOLINT_NEXT_LINE(whitespace/operators)
  col2im_gpu_kernel_half<<<CAFFE_GET_BLOCKS(num_kernels),
                             CAFFE_CUDA_NUM_THREADS>>>(
